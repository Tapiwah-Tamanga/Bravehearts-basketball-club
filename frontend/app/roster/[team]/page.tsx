...
function PlayerCard({
  ...
  return (
    <Link
      ...
      className={`block bg-surface-container-low border border-surface-container-highest group relative overflow-hidden transition-all duration-500 ${teamColorClass} animate-stagger-in`}
      style={{ animationDelay: `${index * 0.1}s` }}
    >
      {/* Team accent top bar */}
      <div className="h-1 w-full" style={{ background: `var(--team-accent)` }} />
      ...
      <div className="aspect-[3/4] relative overflow-hidden bg-surface-container-high">
        ...
        <div className="absolute top-4 right-4">
          <span 
            className="font-stats text-3xl drop-shadow-lg transition-colors duration-300"
        ...
        <div className="absolute top-4 left-4">
          <span 
            className="px-2 py-1 text-xs font-headline font-bold uppercase tracking-wider"
            style={{ 
      ...
      <div className="p-4 bg-surface border-t border-surface-container-highest">
        ...
        {player.threePointPct !== undefined && (
          <div className="flex justify-between mb-3 px-2">
            <span className="font-stats text-xs text-on-surface-variant">
              3P%: <span style={{ color: `var(--team-accent)` }}>{player.threePointPct}</span>
            </span>
            {player.freeThrowPct !== undefined && (
              <span className="font-stats text-xs text-on-surface-variant">
                FT%: <span style={{ color: `var(--team-accent)` }}>{player.freeThrowPct}</span>
              </span>
            )}
          </div>
        )}
        ...
        <div 
          className="w-full py-2.5 font-headline text-xs border text-center transition-all duration-300 group-hover:text-white"
          style={{ 
            borderColor: `var(--team-accent)`,
            color: `var(--team-accent)`
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.background = `var(--team-accent)`;
            e.currentTarget.style.color = 'white';
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.background = 'transparent';
            e.currentTarget.style.color = `var(--team-accent)`;
          }}
        >
          View Profile
        </div>
    ...
    <div className={`flex flex-col min-h-screen ${teamColorClass}`}>
      ...
      <main className="flex-1">
        ...
        <section className="relative min-h-[300px] md:h-[400px] flex items-end bg-surface-container-highest noise-overlay">
          ...
          <div className="relative z-10 px-4 md:px-10 py-8 md:pb-12 w-full">
            <div className="flex flex-wrap items-center gap-3 mb-3 md:mb-4">
              <span 
                className="px-4 py-1.5 font-headline text-xs text-white uppercase tracking-widest"
                style={{ background: `var(--team-accent)` }}
              >
                {teamBadge}
              </span>
              ...
              {isYouthTeam && (
                <span className="px-3 py-1.5 bg-team-youth-container text-team-youth font-headline text-xs uppercase tracking-wider">
                  Development Tier
                </span>
              )}
              {isLadiesTeam && (
                <span className="px-3 py-1.5 bg-team-ladies-container text-team-ladies font-headline text-xs uppercase tracking-wider">
                  Elite Division
                </span>
              )}
            </div>
            ...
            <h2 className="font-headline text-4xl md:text-7xl text-on-surface uppercase leading-none mb-2">
              The <span style={{ color: `var(--team-accent)` }}>{team?.name.split(" ").slice(-1)}</span> Team
            </h2>
        ...
        <section className="px-4 md:px-10 py-8 md:py-12">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 mb-8 md:mb-12">
            <div className="md:col-span-2 bg-surface-container border border-surface-container-highest p-4 md:p-6 relative overflow-hidden group">
              <div 
                className="absolute top-0 right-0 w-32 h-32 opacity-5 -mr-16 -mt-16 rounded-full group-hover:scale-150 transition-transform duration-700"
                style={{ background: `var(--team-accent)` }}
              />
              <p className="font-headline text-xs text-on-surface-variant mb-2 md:mb-4 uppercase tracking-wider">
                Division Win Rate
              </p>
              <p className="font-stats text-3xl" style={{ color: `var(--team-accent)` }}>
                {winRate}%
              </p>
            </div>
            ...
            <div 
              className="p-4 md:p-6 text-white border-l-4"
              style={{ 
                background: `var(--team-accent)`,
        ...
        <section className="px-4 md:px-10 py-8 md:py-12 bg-surface-container-low">
          <div className="flex justify-between items-center mb-8 md:mb-12 border-l-8 pl-4 md:pl-6" style={{ borderColor: `var(--team-accent)` }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            <div className="p-4 md:p-6 text-white text-center" style={{ background: `var(--team-accent)` }}>
        ...
        <section className="px-4 md:px-10 py-8 md:py-12">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 md:mb-12 border-l-8 pl-4 md:pl-6" style={{ borderColor: `var(--team-accent)` }}>
            <h3 className="font-headline text-2xl md:text-4xl uppercase text-on-surface">
              Active Roster
            </h3>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                style={selectedPosition === "ALL" ? { background: `var(--team-accent)` } : {}}
              >
              ...
              {positions.map((pos) => {
                ...
                return (
                  <button
                    ...
                    style={selectedPosition === pos ? { background: `var(--team-accent)` } : {}}
                  >
                    {pos} ({count})
                  </button>
                );
        ...
        {isYouthTeam && (
          <section className="px-4 md:px-10 py-8 md:py-12 bg-surface-container-low">
            <div className="flex justify-between items-center mb-8 md:mb-12 border-l-8 border-team-youth pl-4 md:pl-6">
              <h3 className="font-headline text-2xl md:text-4xl uppercase text-on-surface">
                Scout Booking
              </h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-surface-container border border-surface-container-highest p-6">
                <h4 className="font-headline text-lg uppercase mb-4" style={{ color: `var(--team-accent)` }}>Morning Drill</h4>
                <p className="font-body text-sm text-on-surface-variant mb-4">Available • 8:00 AM - 10:00 AM</p>
                <button 
                  className="w-full py-3 font-headline text-sm uppercase tracking-wider text-white transition-all duration-300 hover:opacity-90"
                  style={{ background: `var(--team-accent)` }}
                >
                  Book Session
                </button>
            ...
            <div className="mt-8 text-center">
              <button 
                className="px-8 py-4 font-headline text-sm uppercase tracking-wider border-2 transition-all duration-300 hover:text-white"
                style={{ 
                  borderColor: `var(--team-accent)`,
                  color: `var(--team-accent)`
                }}
        ...
        {isLadiesTeam && (
          <section className="px-4 md:px-10 py-8 md:py-12 bg-surface-container-low">
            <div className="flex justify-between items-center mb-8 md:mb-12 border-l-8 border-team-ladies pl-4 md:pl-6">
              <h3 className="font-headline text-2xl md:text-4xl uppercase text-on-surface">
                Draft Center
              </h3>
            </div>
            <div className="bg-surface-container border-2 border-dashed border-surface-container-highest p-8 text-center">
              ...
              <button 
                className="px-8 py-3 font-headline text-sm uppercase tracking-wider text-white transition-all duration-300 hover:opacity-90"
                style={{ background: `var(--team-accent)` }}
              >
                Start Scouting
              </button>
        ...
        <section className="px-4 md:px-10 py-8 md:py-12">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-surface-container border-b-2" style={{ borderColor: `var(--team-accent)` }}>
              ...
              <tbody>
                {teamPlayers.map((player) => (
                  <tr key={player.id} className="border-b border-surface-container-highest hover:bg-surface-container transition-colors duration-200">
                    <td className="px-4 py-3 font-stats text-lg" style={{ color: `var(--team-accent)` }}>
                      {String(player.jerseyNumber).padStart(2, "0")}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span 
                        className="px-2 py-0.5 text-xs font-headline font-bold"
                        style={{ 
                          background: `var(--team-accent-container)`,
                          color: `var(--team-accent)`
                        }}
                      >
                        {player.position}
                    ...
                    <td className="px-4 py-3">
                      ...
                      style={player.status === "Starter" ? { background: `var(--team-accent)` } : {}}
                      >
        ...
        <section className="px-4 md:px-10 py-8 md:py-12 bg-surface-container-low">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
            <div className="bg-surface-container border border-surface-container-highest p-4 md:p-6">
              ...
              <p className="font-stats text-2xl" style={{ color: `var(--team-accent)` }}>{trainingStats.offensiveRating}</p>
            </div>
            <div className="bg-surface-container border border-surface-container-highest p-4 md:p-6">
            ...
            <div className="bg-surface-container border border-surface-container-highest p-4 md:p-6">
              ...
              <p className="font-stats text-2xl text-secondary">+{(trainingStats.offensiveRating - trainingStats.defensiveRating).toFixed(1)}</p>
            </div>
            <div className="bg-surface-container border border-surface-container-highest p-4 md:p-6">
            ...
            {recentGames.slice(0, 6).map((game) => (
              ...
              >
                ...
                <p className="font-stats text-lg">
                  <span style={{ color: `var(--team-accent)` }}>{game.scoreFor}</span>
